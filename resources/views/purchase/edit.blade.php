                ...
                <form id="insertForm" action="{{ route('purchases.update', $purchase->id) }}" method="POST"
                            ...
                            <div class="col-md-6 border-right pr-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control @error('purchase_date') is-invalid @enderror"
                                        id="purchase_date" name="purchase_date" value="{{ old('purchase_date', $purchase->purchase_date ? \Carbon\Carbon::parse($purchase->purchase_date)->format('Y-m-d') : date('Y-m-d')) }}" required>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('supplier_id') is-invalid @enderror" name="supplier_id" id="supplier_id" required>
                                        ...
                                        @foreach ($suppliers as $supplier)
                                            <option value="{{ $supplier->id }}" @selected(old('supplier_id', $purchase->supplier_id) == $supplier->id)>{{ $supplier->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('shop_id') is-invalid @enderror" name="shop_id" id="shop_id" required>
                                        ...
                                        @foreach ($shops as $shop)
                                            <option value="{{ $shop->id }}" @selected(old('shop_id', $purchase->shop_id) == $shop->id)>{{ $shop->nama }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('no_so') is-invalid @enderror" id="no_so" name="no_so" value="{{ old('no_so', $purchase->no_so) }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="no_lo">No. LO</label>
                                    <input type="text" class="form-control @error('no_lo') is-invalid @enderror" id="no_lo" name="no_lo" value="{{ old('no_lo', $purchase->no_lo) }}" required>
                                </div>
                                @endif

                                <div class="form-group">
                                    <label for="trip">Trip</label>
                                    <input type="text" class="form-control @error('trip') is-invalid @enderror" id="trip" name="trip" value="{{ old('trip', $purchase->trip) }}">
                                </div>

                                <div class="form-group">
                                    <label for="delivery_date">Tanggal Pengiriman</label>
                                    <input type="date" class="form-control @error('delivery_date') is-invalid @enderror" id="delivery_date" name="delivery_date" value="{{ old('delivery_date', $purchase->delivery_date ? \Carbon\Carbon::parse($purchase->delivery_date)->format('Y-m-d') : date('Y-m-d')) }}">
                                </div>
                            </div>
                            ...
                            <div class="col-md-6 pl-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="number" step="0.01" class="form-control @error('jumlah_kl') is-invalid @enderror" id="jumlah_kl" name="jumlah_kl" value="{{ old('jumlah_kl', $purchase->volume / 1000) }}" required>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control @error('total_nominal') is-invalid @enderror" id="total_nominal" name="total_nominal" value="{{ old('total_nominal', $purchase->total_nominal > 0 ? $purchase->total_nominal : $purchase->total_bayar) }}" required>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-prepend"><span class="input-group-text">Rp</span></div>
                                        <input type="number" class="form-control @error('catatan_debit_credit') is-invalid @enderror" id="catatan_debit_credit" name="catatan_debit_credit" value="{{ old('catatan_debit_credit', $purchase->catatan_debit_credit) }}">
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-3">
                                        ...
                                        <input type="number" step="0.01" class="form-control form-control-sm calc-input" id="persen_net" name="persen_net" value="{{ old('persen_net', $purchase->persen_net ?? 85.06) }}" required>
                                    </div>
                                    <div class="col-3">
                                        <label style="font-size: 0.8rem;">PPN</label>
                                        <input type="number" step="0.01" class="form-control form-control-sm calc-input" id="persen_ppn" name="persen_ppn" value="{{ old('persen_ppn', $purchase->persen_ppn ?? 10.11) }}" required>
                                    </div>
                                    <div class="col-3">
                                        <label style="font-size: 0.8rem;">PPh</label>
                                        <input type="number" step="0.01" class="form-control form-control-sm calc-input" id="persen_pph" name="persen_pph" value="{{ old('persen_pph', $purchase->persen_pph ?? 0.23) }}" required>
                                    </div>
                                    <div class="col-3">
                                        <label style="font-size: 0.8rem;">PBBKB</label>
                                        <input type="number" step="0.01" class="form-control form-control-sm calc-input" id="persen_pbbkb" name="persen_pbbkb" value="{{ old('persen_pbbkb', $purchase->persen_pbbkb ?? 4.60) }}" required>
                                    </div>
                                </div>
                    ...
                    <div class="card-footer bg-white border-top">
                        <div class="d-flex justify-content-end">
                            ...
                            <button type="submit" class="btn btn-primary px-4"><i class="fas fa-save mr-2"></i> Simpan Perubahan DO</button>
                        </div>
                    </div>
                </form>
            </div>
