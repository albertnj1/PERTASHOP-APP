...
@section('title', 'Edit Sistem Penggajian')
...
<div class="d-flex align-items-center justify-content-between mb-4">
  <div>
    <h1 class="page-title mb-1" style="font-size: 24px; font-weight: 900; color: #0f172a;">Edit Sistem Penggajian</h1>
    <p class="text-muted mb-0" style="font-size: 13px;">Ubah aturan kompensasi dan parameter penggajian untuk cabang {{ $payrollSystem->shop->nama }}</p>
  </div>
  ...
  <div class="container-fluid px-0">
    <div class="row justify-content-center">
      <div class="col-lg-10">
        ...
        <div class="alert alert-warning mb-4" style="border-radius: 12px; border: 1px solid #fef08a; background: #fefce8; color: #854d0e;">
          <i class="fas fa-exclamation-triangle mr-1"></i>
          <strong>Catatan:</strong> Perubahan parameter di sini akan berlaku untuk kalkulasi periode berikutnya atau saat periode draft di-generate ulang.
        </div>
        ...
        <div class="card shadow-sm" style="border-radius: 16px; border: 1px solid #e2e8f0;">
          <div class="card-header bg-white py-3" style="border-bottom: 1px solid #f1f5f9;">
            <h5 class="mb-0 font-weight-bold" style="color: #0f172a;">
              <i class="fas fa-edit text-success mr-2"></i>Edit Pengaturan: {{ $payrollSystem->nama_sistem }}
            </h5>
          </div>
          <div class="card-body p-4">
            ...
            <form action="{{ route('payroll-systems.update', $payrollSystem->id) }}" method="POST" id="form-payroll-system">
              @csrf
              @method('PUT')
              ...
              <div class="form-group mb-4">
                ...
                <select name="shop_id" class="form-control form-control-lg font-weight-bold" required>
                  @foreach($shops as $shop)
                    <option value="{{ $shop->id }}" {{ old('shop_id', $payrollSystem->shop_id) == $shop->id ? 'selected' : '' }}>
                      {{ $shop->nama }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="form-group mb-4">
                ...
                <input type="text" name="nama_sistem" class="form-control" 
                  value="{{ old('nama_sistem', $payrollSystem->nama_sistem) }}" required>
              </div>

              {{-- 3. PEMILIHAN TIPE SKEMA PENGGAJIAN (3 PILIHAN) --}}
              @php
                $activeScheme = old('tipe_skema', $payrollSystem->tipe_skema ?? 'komisi_murni');
              @endphp
              <div class="form-group mb-4">
                ...
                <div class="row">
                  ...
                  <div class="col-md-4 mb-3">
                    <div class="scheme-selector-card {{ $activeScheme === 'komisi_murni' ? 'active' : '' }}" 
                      ...
                      <input type="radio" name="tipe_skema" value="komisi_murni" id="radio-komisi_murni"
                             {{ $activeScheme === 'komisi_murni' ? 'checked' : '' }}>
                  ...
                  <div class="col-md-4 mb-3">
                    <div class="scheme-selector-card {{ $activeScheme === 'gaji_pokok_murni' ? 'active' : '' }}" 
                      ...
                      <input type="radio" name="tipe_skema" value="gaji_pokok_murni" id="radio-gaji_pokok_murni"
                             {{ $activeScheme === 'gaji_pokok_murni' ? 'checked' : '' }}>
                      <div class="d-flex align-items-center mb-2">
              ...
              <div class="p-3 mb-4" style="background: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0;">
                  ...
                  <div class="form-group col-md-6" id="wrap-rate-liter">
                    ...
                    <div class="input-group">
                      ...
                      <input type="number" name="rate_per_liter" id="input_rate_per_liter" class="form-control font-weight-bold" 
                             value="{{ old('rate_per_liter', $payrollSystem->rate_per_liter) }}" min="0" step="1">
                    </div>
                  ...
                  <div class="form-group col-md-6" id="wrap-gaji-pokok">
                    ...
                    <div class="input-group">
                      ...
                      <input type="number" name="nominal_gaji_pokok" id="input_nominal_gaji_pokok" class="form-control font-weight-bold" 
                             value="{{ old('nominal_gaji_pokok', $payrollSystem->nominal_gaji_pokok) }}" min="0" step="1000">
                    </div>
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <div class="input-group">
                      ...
                      <input type="number" name="rate_transport_per_hari" class="form-control" 
                             value="{{ old('rate_transport_per_hari', $payrollSystem->rate_transport_per_hari ?? 0) }}" min="0" step="1000">
                    </div>
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <div class="input-group">
                      ...
                      <input type="number" name="potongan_per_hari_alpha" class="form-control" 
                             value="{{ old('potongan_per_hari_alpha', $payrollSystem->potongan_per_hari_alpha) }}" min="0" step="1000">
                    </div>
                ...
                <div class="form-group mb-0">
                  ...
                  <select name="metode_potongan_alpha" class="form-control" required>
                    <option value="nominal_tetap" {{ old('metode_potongan_alpha', $payrollSystem->metode_potongan_alpha ?? 'nominal_tetap') === 'nominal_tetap' ? 'selected' : '' }}>
                      Nominal Tetap (Pakai input Potongan Alpha per Hari di atas)
                    </option>
                    <option value="prorata_gaji_pokok" {{ old('metode_potongan_alpha', $payrollSystem->metode_potongan_alpha) === 'prorata_gaji_pokok' ? 'selected' : '' }}>
              ...
              <div class="form-row">
                <div class="form-group col-md-6">
                  ...
                  <select name="perlakuan_losses_gain" class="form-control" required>
                    <option value="losses_only" {{ old('perlakuan_losses_gain', $payrollSystem->perlakuan_losses_gain) === 'losses_only' ? 'selected' : '' }}>
                      Losses Saja (Gain Diabaikan)
                    </option>
                    <option value="losses_dan_gain" {{ old('perlakuan_losses_gain', $payrollSystem->perlakuan_losses_gain) === 'losses_dan_gain' ? 'selected' : '' }}>
                      Losses &amp; Gain (Plus/Minus)
                    </option>
                    <option value="abaikan_losses_gain" {{ old('perlakuan_losses_gain', $payrollSystem->perlakuan_losses_gain) === 'abaikan_losses_gain' ? 'selected' : '' }}>
                ...
                <div class="form-group col-md-6">
                  ...
                  <select name="metode_split" class="form-control" required onchange="toggleStandarHariKerja(this)">
                    <option value="per_hari_penuh" {{ old('metode_split', $payrollSystem->metode_split) === 'per_hari_penuh' ? 'selected' : '' }}>
                      Satu Hari Penuh (Shift Berdasarkan Kehadiran Riil)
                    </option>
                    <option value="proporsional_jam_kerja" {{ old('metode_split', $payrollSystem->metode_split) === 'proporsional_jam_kerja' ? 'selected' : '' }}>
                      Proporsional Jam Kerja per Shift
                    </option>
                    <option value="flat_bulanan_prorata_hari" {{ old('metode_split', $payrollSystem->metode_split) === 'flat_bulanan_prorata_hari' ? 'selected' : '' }}>
              ...
              <div class="form-group" id="standar-hari-wrap" style="{{ old('metode_split', $payrollSystem->metode_split) === 'flat_bulanan_prorata_hari' ? '' : 'display:none;' }}">
                ...
                <input type="number" name="standar_hari_kerja" class="form-control"
                  value="{{ old('standar_hari_kerja', $payrollSystem->standar_hari_kerja ?? 26) }}" min="1" max="31">
              ...
              <div class="form-group mb-4">
                <div class="custom-control custom-switch">
                  <input type="checkbox" class="custom-control-input" id="aktif"
                    name="aktif" value="1" {{ old('aktif', $payrollSystem->aktif) ? 'checked' : '' }}>
              ...
              <div class="d-flex align-items-center pt-3 border-top">
                <button type="submit" class="btn btn-success px-4 py-2" style="border-radius: 10px; font-weight: 700;">
                  <i class="fas fa-save mr-1"></i> Perbarui Sistem Penggajian
                </button>
